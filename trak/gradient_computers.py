...
class AbstractGradientComputer(ABC):
    ...
    @abstractmethod
    def __init__(self, model: torch.nn.Module, task: AbstractModelOutput, grad_dim: Optional[int] = None,
                 dtype: Optional[torch.dtype] = torch.float16, device: Optional[torch.device] = "cuda",) -> None:
...
class IterativeGradientComputer(AbstractGradientComputer):
    ...
    def compute_per_sample_grad(self, batch: Iterable[Tensor]) -> Tensor:
        ...
        grads = ch.zeros(batch_size, self.grad_dim).to(images.device)
        ...
        with torch.enable_grad():
            ...
            for ind in range(batch_size):
                # 获取当前样本的logit（需要梯度的张量）
                sample_logit = logits[ind]

                # 计算该样本的梯度
                with torch.backends.cudnn.flags(enabled=False, deterministic=False):  # 明确设置
                    grad_per_sample = ch.autograd.grad(
                        outputs=sample_logit,  # 传入张量而非整数
                        inputs=self.model_params,
                        grad_outputs=torch.ones_like(sample_logit),  # 提供梯度输出
                        retain_graph=True,
                        allow_unused=True
                    )

                # 处理未使用的梯度
                grad_per_sample = [g if g is not None else torch.zeros_like(p)
                                   for g, p in zip(grad_per_sample, self.model_params)]

                # 向量化梯度
                grads[ind] = parameters_to_vector(grad_per_sample)
