...
class BasicScoreComputer(AbstractScoreComputer):
    ...
    def get_xtx(self, grads: Tensor) -> Tensor:
        ...
        # 使用字符串比较设备类型
        compute_dtype = torch.float32 if self.device == 'cpu' else self.dtype

        result = torch.zeros(
            self.proj_dim, self.proj_dim, dtype=compute_dtype, device=self.device
        )
        blocks = torch.split(grads, split_size_or_sections=self.CUDA_MAX_DIM_SIZE, dim=0)

        for block in blocks:
            block = block.to(self.device).to(compute_dtype)  # 转换为计算精度
            result += block.T @ block

        return result

    # 在 get_x_xtx_inv 方法中添加类型转换
    def get_x_xtx_inv(self, grads: Tensor, xtx: Tensor) -> Tensor:
        ...
        # 确保在CPU上使用float32进行计算
        compute_dtype = torch.float32  # 使用单精度进行矩阵求逆
        xtx_reg = xtx_reg.to(compute_dtype)
        xtx_inv = ch.linalg.inv(xtx_reg)

        result = ch.empty(
            grads.shape[0], xtx_inv.shape[1], dtype=grads.dtype, device=self.device
        )
        ...
        for i, block in enumerate(blocks):
            ...
            xtx_inv = xtx_inv.to(compute_dtype)
            res_slice = block @ xtx_inv

            # 转换回原始数据类型
            result[start:end] = res_slice.to(grads.dtype)
