...
# 添加的代码，算CTC
class OCRCTCModelOutput(AbstractModelOutput):
    def __init__(self, blank_token=0):
        super().__init__()
        self.blank_token = blank_token
        self.ctc_loss = CTCLoss(blank=blank_token, reduction='none', zero_infinity=True)

    def get_output(self, model, weights, buffers, images, labels, label_lengths) -> Tensor:
        # 获取输入设备
        device = images.device  # 关键修复：从输入张量获取设备信息
        ...
        # 修改返回值处理
        loss = -loss  # 保持原始符号
        loss.requires_grad_(True)  # 确保保留梯度信息
        return loss
    ...
    def get_out_to_loss_grad(self, model, weights, buffers, batch) -> Tensor:
        ...
        # 返回-1作为out-to-loss梯度
        return -torch.ones(batch_size, 1, device=images.device)
