...
class BasicProjector(AbstractProjector):
    ...
    def generate_sketch_matrix(self, generator_state):
        ...
        if not self.proj_matrix_available:
            self.proj_matrix = ch.empty(
                self.grad_dim, self.block_size,
                dtype=ch.float32 if self.device == 'cpu' else self.dtype,
                device=self.device
            )
            self.proj_matrix_available = True
        ...
        # 直接检查设备字符串而不是访问.type属性
        if self.device == 'cpu':
            work_tensor = self.proj_matrix.float()
        else:
            work_tensor = self.proj_matrix

        if self.proj_type == ProjectionType.normal or self.proj_type == "normal":
            work_tensor.normal_(generator=self.generator)
        elif (self.proj_type == ProjectionType.rademacher or
            self.proj_type == "rademacher"):
            work_tensor.bernoulli_(p=0.5, generator=self.generator)
            work_tensor = work_tensor * 2.0 - 1.0

        # 将结果转换回目标数据类型
        if self.device == 'cpu':
            self.proj_matrix.copy_(work_tensor.to(dtype=self.dtype))
        else:
            self.proj_matrix = work_tensor.to(dtype=self.dtype)
