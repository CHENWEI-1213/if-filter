...
class TRAKer:
    ...
    def __init__(
        ...
        self.gradient_computer = gradient_computer(model=self.model, task=self.task, grad_dim=self.num_params_for_grad,
                                                   dtype=self.dtype, device=self.device, grad_wrt=self.grad_wrt,)
    ...
    def init_projector(self, projector: Optional[AbstractProjector], proj_dim: int, proj_max_batch_size: int,) -> None:
        ...
        if self.device == "cpu":
            use_half_precision = False  # 确保与原始逻辑一致
            self.dtype = torch.float32  # 同步更新 dtype
        ...
        if projector is not None:
            ...
            proj_type = (
                ...
                if self.device == "cpu"
                else ProjectionType.rademacher
            )
            ...
            if self.device == "cpu":
                ...
                proj_type = ProjectionType.normal if self.device == "cpu" else ProjectionType.rademacher
    ...
    def load_checkpoint(self, checkpoint: Iterable[Tensor], model_id: int, _allow_featurizing_already_registered=False,) -> None:
    ...
    def featurize(self, batch: Iterable[Tensor], inds: Optional[Iterable[int]] = None, num_samples: Optional[int] = None,) -> None:
        ...
        # ===== 关键修改开始 =====
        # 确保在梯度计算上下文管理器内执行
        with torch.backends.cudnn.flags(enabled=False):  # 临时禁用CuDNN
            with torch.autocast(device_type='cuda', enabled=False):  # 临时禁用自动混合精度
                # 在非确定性模式下计算梯度
    ...
    def start_scoring_checkpoint(self, exp_name: str, checkpoint: Iterable[Tensor], model_id: int, num_targets: int,) -> None:
    ...
    def score(self, batch: Iterable[Tensor], inds: Optional[Iterable[int]] = None, num_samples: Optional[int] = None,) -> None:
    ...
    def finalize_scores(self, exp_name: str, model_ids: Iterable[int] = None, allow_skip: bool = False,) -> Tensor:
